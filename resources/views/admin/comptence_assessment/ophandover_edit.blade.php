            ...
            <div class="col-md-8">
                ...
                <div class="box">
                    ...
                    <div class="box-body">
                        ...
                        <form action="{{ route('ophandover.update',[$ophandover->id,$ctass->slug]) }}" method="post"
                            onsubmit="return confirm ('Do you want to submit the edited entries you made in this section?')">
                            @csrf


                            <input type="hidden" name="competenceassessment_id" value="{{ $ophandover->competenceassessment_id }}">
                            <input type="hidden" name="user_id" value="{{ Auth::user()->id }}">
                            <input type="hidden" name="caption"
                                value="Operations Handover">
                            ...
                            <div class="form-group">
                                ...
                                <select name="legend_id" class="form-control" required>
                                    ...
                                    <option value="{{ $level->id }}"
                                        {{ $level->id==$ophandover->legend_id?'selected':'' }}>{{ $level->name }}
                                    </option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <textarea name="evidence" class="form-control" cols="30" rows="3" required
                                    placeholder="Enter evidence">{{ $ophandover->evidence }}</textarea>
                            </div>
