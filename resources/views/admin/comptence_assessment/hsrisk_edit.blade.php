            ...
            <div class="col-md-8">
                ...
                <div class="box">
                    ...
                    <div class="box-body">
                        ...
                        <form action="{{ route('hsrisk.update',[$hsrisk->id,$ctass->slug]) }}" method="post"
                            onsubmit="return confirm ('Do you want to submit the edited entries you made in this section?')">
                            @csrf


                            <input type="hidden" name="competenceassessment_id" value="{{ $hsrisk->competenceassessment_id }}">
                            <input type="hidden" name="user_id" value="{{ Auth::user()->id }}">
                            <input type="hidden" name="caption"
                                value="Health & Safety - Identify Risk">
                            ...
                            <div class="form-group">
                                ...
                                <select name="legend_id" class="form-control" required>
                                    ...
                                    <option value="{{ $level->id }}"
                                        {{ $level->id==$hsrisk->legend_id?'selected':'' }}>{{ $level->name }}
                                    </option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <textarea name="evidence" class="form-control" cols="30" rows="3" required
                                    placeholder="Enter evidence">{{ $hsrisk->evidence }}</textarea>
                            </div>
