            ...
            <div class="col-md-8">
                ...
                <div class="box">
                    ...
                    <div class="box-body">
                        ...
                        <form action="{{ route('forkliftop.update',[$forkliftop->id,$ctass->slug]) }}" method="post"
                            onsubmit="return confirm ('Do you want to submit the edited entries you made in this section?')">
                            @csrf


                            <input type="hidden" name="competenceassessment_id" value="{{ $forkliftop->competenceassessment_id }}">
                            ...
                            <div class="form-group">
                                ...
                                <select name="legend_id" class="form-control" required>
                                    ...
                                    <option value="{{ $level->id }}"
                                        {{ $level->id==$forkliftop->legend_id?'selected':'' }}>{{ $level->name }}
                                    </option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <textarea name="evidence" class="form-control" cols="30" rows="3" required
                                    placeholder="Enter evidence">{{ $forkliftop->evidence }}</textarea>
                            </div>
