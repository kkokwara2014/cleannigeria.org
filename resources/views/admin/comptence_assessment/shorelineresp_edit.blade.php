            ...
            <div class="col-md-8">
                ...
                <div class="box">
                    ...
                    <div class="box-body">
                        ...
                        <form action="{{ route('shorelineresp.update',[$shorelineresp->id,$ctass->slug]) }}" method="post"
                            onsubmit="return confirm ('Do you want to submit the edited entries you made in this section?')">
                            @csrf


                            <input type="hidden" name="competenceassessment_id" value="{{ $shorelineresp->competenceassessment_id }}">
                            <input type="hidden" name="user_id" value="{{ Auth::user()->id }}">
                            <input type="hidden" name="caption"
                                value="Shoreline Response">
                            ...
                            <div class="form-group">
                                ...
                                <select name="legend_id" class="form-control" required>
                                    ...
                                    <option value="{{ $level->id }}"
                                        {{ $level->id==$shorelineresp->legend_id?'selected':'' }}>{{ $level->name }}
                                    </option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <textarea name="evidence" class="form-control" cols="30" rows="3" required
                                    placeholder="Enter evidence">{{ $shorelineresp->evidence }}</textarea>
                            </div>
